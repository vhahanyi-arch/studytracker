// Papers saved before question screenshots existed (2026-09-24) have no
// crops, so every question showed its whole page, which made answering hard;
// papers saved before a later change to how crops are worked out have older
// ones (see CROPS_VERSION in exam-paper-layout.ts). Either way the crops are
// worked out again the first time the paper is loaded and saved, so it only
// happens once. Best effort: if the question paper cannot be read, the paper
// is served as before and the next load tries again.
//
// Multiple-choice papers are left alone: their crops come from their own
// reading (multipleChoicePaper), which none of this changes.
import type { Paper } from "./physics-extraction-schema";

type Crops = NonNullable<Paper["crops"]>;

// Papers from before versions were recorded count as version 1 if they have crops.
export const cropsVersionOf = (paper: Paper) => paper.cropsVersion ?? (paper.crops ? 1 : 0);

export async function addCurrentScreenshots(
  papers: Paper[],
  version: number,
  find: (paper: Paper) => Promise<Crops | undefined>,
  save: (id: string, crops: Crops, version: number) => Promise<void>,
  log: (line: string) => void = console.warn,
): Promise<Paper[]> {
  return Promise.all(papers.map(async (paper) => {
    if (paper.kind === "multiple_choice" || cropsVersionOf(paper) >= version) return paper;
    const crops = await find(paper);
    if (!crops) {
      log(`[screenshots] could not place the questions of paper ${paper.id}; keeping what it had`);
      return paper;
    }
    try {
      await save(paper.id, crops, version);
    } catch (error) {
      log(`[screenshots] placed but could not save for paper ${paper.id}: ${error instanceof Error ? error.message : String(error)}`);
    }
    return { ...paper, crops, cropsVersion: version };
  }));
}
