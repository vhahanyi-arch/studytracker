...
// Gives a paper current screenshot crops, when it has none or older ones
// (see lib/exam-paper-screenshots.ts). Only the crops and their version are
// written, and only while the saved ones are still older, so a teacher's
// edit made meanwhile is never overwritten.
export async function setOutdatedPaperCrops(id: string, crops: NonNullable<Paper["crops"]>, version: number): Promise<void> {
  await sql`
    UPDATE physics_exam_papers
    SET payload = payload || ${JSON.stringify({ crops, cropsVersion: version })}::jsonb
    WHERE id=${id}
      AND COALESCE((payload->>'cropsVersion')::int, CASE WHEN payload ? 'crops' THEN 1 ELSE 0 END) < ${version}
  `;
}
