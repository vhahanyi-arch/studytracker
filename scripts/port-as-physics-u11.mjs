// Run only after the standalone Particles draft regression passes.
import assert from 'node:assert/strict';
import fs from 'node:fs';
const path='lib/physics-question-engine.ts';
let engine=fs.readFileSync(path,'utf8').replace(/\r\n/g,'\n');
let body=fs.readFileSync('scripts/as-physics-u11.draft.mjs','utf8').replace(/\r\n/g,'\n').split('// BEGIN VERIFIED BODY\n')[1].split('// END VERIFIED BODY')[0];
body=body.replace('const decays=','const decays: [number,number,string,string,string][]=')
  .replace('const speciesList=','const speciesList: [string,number,number][]=')
  .replace('const structuredAsParticles = (difficulty) =>','const structuredAsParticles = (difficulty: "foundational"|"application"|"reasoning"): PhysicsQuestion[] =>')
...
const start='// BEGIN AS TOPIC 11\n',end='// END AS TOPIC 11\n';
if(engine.includes(start))engine=engine.slice(0,engine.indexOf(start))+engine.slice(engine.indexOf(end)+end.length).replace(/^\n/,'');
assert.ok(engine.includes('const asTopics:'));
engine=engine.replace('const asTopics:',start+body+end+'\nconst asTopics:');
engine=engine.replace(/  "as-u11": structuredAsParticles,\n/g,'');
engine=engine.replace('  "as-u10": structuredAsCircuits,\n','  "as-u10": structuredAsCircuits,\n  "as-u11": structuredAsParticles,\n');
assert.match(engine,/"as-u11": structuredAsParticles/);
