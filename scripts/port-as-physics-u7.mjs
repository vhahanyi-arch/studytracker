// Run only after the standalone Waves draft regression passes.
import assert from 'node:assert/strict';
import fs from 'node:fs';
const path='lib/physics-question-engine.ts';
let engine=fs.readFileSync(path,'utf8').replace(/\r\n/g,'\n');
let body=fs.readFileSync('scripts/as-physics-u7.draft.mjs','utf8').replace(/\r\n/g,'\n').split('// BEGIN VERIFIED BODY\n')[1].split('// END VERIFIED BODY')[0];
body=body.replace('const structuredAsWaves = (difficulty) =>','const structuredAsWaves = (difficulty: "foundational"|"application"|"reasoning"): PhysicsQuestion[] =>')
...
assert.match(engine,/"as-u7": structuredAsWaves/);
