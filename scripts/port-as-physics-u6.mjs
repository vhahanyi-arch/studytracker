// Run only after the standalone Deformation draft regression passes.
import assert from 'node:assert/strict';
import fs from 'node:fs';
const path='lib/physics-question-engine.ts';
let engine=fs.readFileSync(path,'utf8').replace(/\r\n/g,'\n');
let body=fs.readFileSync('scripts/as-physics-u6.draft.mjs','utf8').replace(/\r\n/g,'\n').split('// BEGIN VERIFIED BODY\n')[1].split('// END VERIFIED BODY')[0];
body=body.replace('const structuredAsDeformation = (difficulty) =>','const structuredAsDeformation = (difficulty: "foundational"|"application"|"reasoning"): PhysicsQuestion[] =>')
...
engine=engine.replace(/  "as-u6": structuredAsDeformation,\n/g,'');
engine=engine.replace('  "as-u5": structuredAsEnergy,\n','  "as-u5": structuredAsEnergy,\n  "as-u6": structuredAsDeformation,\n');
assert.match(engine,/"as-u6": structuredAsDeformation/);
