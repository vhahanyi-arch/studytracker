...
function Stage89Teacher({ stage }: { stage: 8 | 9 }) {
  ...
  const [progress, setProgress] = useState<Array<{student_id:string;student_name:string;source_stage:number;chapter_id:string;past_paper?:boolean;attempts:number;average:number;strong_sets:number;mastered:boolean}>>([]);
...
function Stage89Student({ back }:{ back:()=>void }) {
  ...
  const [session,setSession]=useState<{id:string;source_stage:number;difficulty:string;past_paper?:boolean;questions:Array<{templateId?:string;objective?:string;difficulty?:string;answerFormat?:string;prompt:string;hint:string;source?:PastPaperPracticeSource}>}|null>(null);
  const [answers,setAnswers]=useState<string[]>([]),[hints,setHints]=useState<boolean[]>([]),[cursor,setCursor]=useState(0),[result,setResult]=useState<any>(null),[message,setMessage]=useState("");
  ...
  return <>
    ...
    <section className="panel progress-units">
      ...
      {units.map((unit) => <article key={unit.source + unit.track + unit.chapterId + (unit.pastPaper ? ":past" : "")}>
        ...
        <strong className={unit.mastered ? "mastered" : ""}>{unit.pastPaper ? "Practice only" : unit.mastered ? "Mastered" : unit.strongSets + "/2 strong"}</strong>
      </article>)}
    </section>
  </>;
}
